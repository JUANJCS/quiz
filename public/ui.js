...
function toggleAnswer(s){
  if (s) stage.classList.add('show-answer');
...
function goNext(){
  ...
  if (idx < items.length - 1){
    idx++; answered = false; setFeedback(''); toggleAnswer(false);
    $('next').disabled = true; enableChoices(true); resetCard(); render(); setStats(idx);
  } else {
...
function finishSwipe(){ if (!aDrag) return; aDrag = false; const dx = aCur - aStart; if (Math.abs(dx) >= 60) goNext(); }
panel.addEventListener('pointerup', finishSwipe);
panel.addEventListener('pointercancel', finishSwipe);
panel.addEventListener('pointerleave', finishSwipe);
